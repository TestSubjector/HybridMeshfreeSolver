...
function createGlobalLocalMapIndex(global_local_map_index, folder_name::String)
    index_flag = 1
    for iter in 1:length(workers())
        ...
        @showprogress 1 "Computing IndexReadFile" for (idx, itm) in enumerate(splitdata)
            if idx == 1
                continue
            elseif idx <= local_point_count + 1
                itmdata = split(itm, " ")
                global_local_map_index[parse(Int,itmdata[1])] = index_flag
                index_flag += 1
            end
        end
    end
...
function readGhostFile(folder_name::String, ghost_holder, global_local_map_index)
    ...
    for iter in 1:length(workers())
        ...
        data = read(filename, String)
        splitdata = @view split(data, "\n")[1:end-1]
        ghost_holder[iter] = Dict{Int64,Int64}()
        itmdata = split(splitdata[1], " ")
        local_point_count = parse(Int,itmdata[3])
        ghost_point_count = parse(Int,itmdata[4])

        @showprogress 3 "Computing GhostReadFile" for (idx, itm) in enumerate(splitdata)
            if idx > local_point_count + 1
                itmdata = split(itm, " ")
                ghost_holder[iter][idx-1] = global_local_map_index[parse(Int,itmdata[1])]
            end
        end
    end
end
...
function readDistribuedFile(folder_name::String, local_points_holder, defprimal, p)
    ...
    for iter in p-1:p-1
        ...
        @showprogress 2 "Computing LocalReadFile $(p-1)" for (idx, itm) in enumerate(splitdata)
